...
/* 
 * This is the model file for the uploads controller
 */

// Add image information to the database table
function storeImages($invId, $imgPath, $imgName){ 
 $db = acmeConnect();
...
function getImagesTn($invId){
 $db = acmeConnect();
 $sql = 'SELECT imgId, imgPath, imgName, imgDate, inventory.invId, invName FROM images JOIN inventory ON images.invId = inventory.invId WHERE (imgPath LIKE "%-tn%") AND (inventory.invId = :invId)';
 $stmt = $db->prepare($sql);
 $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
 $stmt->execute();
 $imageArrayTn = $stmt->fetchAll(PDO::FETCH_ASSOC);
 $stmt->closeCursor();
